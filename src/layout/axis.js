...
export function fitAxis(intervals, options = {}) {
  ...
  const padEndMin = options.padEndMin ?? 0
  ...
  // Breathing room either side of the real events. Applied before snapping, so the
  // result still lands on whole hours. Skipped when there is nothing to pad around —
  // the fallback window is already a sensible size.
  if (intervals.length > 0) {
    startMin -= padStartMin
    endMin += padEndMin
  }
